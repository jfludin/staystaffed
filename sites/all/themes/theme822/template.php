 ...
 * Contains theme override functions and process & preprocess functions for theme822
...
if (theme_get_setting('theme822_clear_registry')) {
...
function theme822_html_head_alter(&$head_elements) {
...
function theme822_preprocess_search_block_form(&$vars) {
...
function theme822_preprocess(&$vars, $hook) {

  $vars['theme822_path'] = base_path() . path_to_theme();
...
function theme822_preprocess_html(&$vars) {

  

  $vars['doctype'] = _theme822_doctype();

  $vars['rdf'] = _theme822_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      ...
      $vars['classes_array'][] = theme822_id_safe('page-' . $page_name);

    }



    $vars['classes_array'][] = theme822_id_safe('section-' . $section);
...
function theme822_preprocess_page(&$vars) {
...
function theme822_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme822') . '/css/maintenance-page.css');
...
function theme822_preprocess_node(&$vars) {
...
function theme822_preprocess_block(&$vars, $hook) {
...
function theme822_menu_tree($vars) {
...
function theme822_field__taxonomy_term_reference($vars) {
...
function theme822_breadcrumb($vars) {
...
// Determine if we are to display the breadcrumb.
  ...
  $show_breadcrumb = theme_get_setting('theme822_breadcrumb_display');

  if ($show_breadcrumb == 'yes') {

  

	  if (theme_get_setting('theme822_breadcrumb_hideonlyfront')) {
	  ...
	  if(theme_get_setting('theme822_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme822_breadcrumb_separator');
...
function theme822_tabs_float() {

  $float = (bool) theme_get_setting('theme822_tabs_float');

  $float_node = (bool) theme_get_setting('theme822_tabs_node');
...
function theme822_id_safe($string) {
...
function theme822_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme822_id_safe($element['#title']);
...
function theme822_preprocess_menu_local_task(&$vars) {
...
function theme822_menu_local_tasks(&$vars) {
 ...
 * @ingroup themable
 */
function theme822_follow_link($variables) {
  $link = $variables['link'];
